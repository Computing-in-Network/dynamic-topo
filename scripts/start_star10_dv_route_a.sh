...
stop_managed "push_dv_neighbor_state"
if [[ "$START_STREAM_SERVER" == "1" ]]; then
  stop_managed "stream_server"
fi

if pgrep -af 'scripts/push_static_routes.py' >/dev/null 2>&1; then
  echo "[fatal] detected running push_static_routes.py. Stop the 300-node static control plane before starting DV Route A." >&2
  exit 1
fi
...
"$PYTHON_BIN" scripts/push_sim_policy.py \
  --ws-url "$WS_URL" \
...
"$PYTHON_BIN" scripts/push_dv_neighbor_state.py \
  --ws-url "$WS_URL" \
  --mapping-csv "$SUBSET_MAPPING_CSV" \
  --max-nodes "$NODE_COUNT" \
  --min-stable-frames "$MIN_STABLE_FRAMES" \
  --state-output "$NEIGHBOR_SNAPSHOT_PATH" \
  --once
...
start_bg "push_sim_policy" \
  ...
  --sim-policy-path /opt/sim/policy.json \
  --min-stable-frames "$MIN_STABLE_FRAMES" \
  --min-apply-interval-s "$SIM_POLICY_APPLY_INTERVAL_S"

start_bg "push_dv_neighbor_state" \
  "$PYTHON_BIN" -u scripts/push_dv_neighbor_state.py \
  --ws-url "$WS_URL" \
  --mapping-csv "$SUBSET_MAPPING_CSV" \
  --max-nodes "$NODE_COUNT" \
  --min-stable-frames "$MIN_STABLE_FRAMES" \
  --min-apply-interval-s "$NEIGHBOR_APPLY_INTERVAL_S" \
  --state-output "$NEIGHBOR_SNAPSHOT_PATH"
