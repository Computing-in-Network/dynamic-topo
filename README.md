...
```bash
# 当前脚本会先采样实时拓扑，按最大连通分量抽取连通子图，
# 然后默认按 110 / 120 / 130 / 140 / 145 / 150 ... 继续上探
./scripts/scale_route_a_dv.sh

# 查看逐级验证结果
cat run/route_a_dv_scale/results.tsv
```

当前实验结论：

- 已补上“按实时拓扑最大连通分量选子图”的扩容脚本能力
- `110` 节点可以从瞬时连通分量中启动，端点可达
- 但在持续动态更新下，该 `110` 节点子集会再次分裂，无法长期保持整个子集全连通
- 因此目前可稳定宣称通过的严格规模仍是 `100` 节点
- 如需继续推进到 `110+ / 300`，建议先：
  - 冻结拓扑后做纯规模压测，或
  - 改成按多帧稳定核心节点集合选子图
